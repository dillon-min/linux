...
static int dma2d_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	v4l2_dbg(1, debug, &ctx->dev->v4l2_dev, "sctrl id 0x%x, val 0x%x\n",
		ctrl->id, ctrl->val);
...
static int dma2d_open(struct file *file)
{
	...
	v4l2_dbg(1, debug, &dev->v4l2_dev, "instance opened\n");
	return 0;
}
...
static int dma2d_release(struct file *file)
{
	...
	v4l2_dbg(1, debug, &dev->v4l2_dev, "instance closed\n");
	return 0;
}
...
static int vidioc_g_fmt(struct file *file, void *prv, struct v4l2_format *f)
{
	...
	v4l2_dbg(1, debug, &ctx->dev->v4l2_dev, "gfmt width %d, height %d, \
		pixelformat 0x%x, beytesperline %d, sizeimage %d\n",
...
static int vidioc_try_fmt(struct file *file, void *prv, struct v4l2_format *f)
{
	...
	v4l2_dbg(1, debug, &ctx->dev->v4l2_dev, "try fmt width %d, height %d, \
		pixelformat 0x%x, beytesperline %d, sizeimage %d\n",
		f->fmt.pix.width, f->fmt.pix.height, f->fmt.pix.pixelformat,
		f->fmt.pix.bytesperline, f->fmt.pix.sizeimage);
	return 0;
...
static int vidioc_s_fmt(struct file *file, void *prv, struct v4l2_format *f)
{
	...
	frm->op_mode	= DMA2D_MODE_R2M;//DMA2D_MODE_M2M_FPC;
	frm->fmt	= fmt;
	v4l2_dbg(1, debug, &dev->v4l2_dev, "sfmt width %d, height %d, \
		pixelformat 0x%x, beytesperline %d, sizeimage %d\n",
		f->fmt.pix.width, f->fmt.pix.height, f->fmt.pix.pixelformat,
		f->fmt.pix.bytesperline, f->fmt.pix.sizeimage);
	return 0;
...
static void device_run(void *prv)
{
	...
	if (0 != clk_enable(dev->gate)) {
	    v4l2_err(&dev->v4l2_dev, "can't enable gate clk\n");
	    return;
	} else 
	    v4l2_info(&dev->v4l2_dev, "enable gate clk ok\n");
	...
	dma2d_start(dev);
#else
	//dma2d_config_in(dev, &ctx->out,
	//		vb2_dma_contig_plane_dma_addr(&src->vb2_buf, 0));
	dma2d_config_out(dev, &ctx->in,
			/*(dma_addr_t)vb2_plane_vaddr(&dst->vb2_buf, 0)*/vb2_dma_contig_plane_dma_addr(&src->vb2_buf, 0));
	dma2d_start(dev);
#endif
...
static irqreturn_t dma2d_isr(int irq, void *prv)
{
	...
	if (dma2d_get_int(dev) & ISR_TCIF) {
	    ...
	    v4l2_dbg(1, debug, &dev->v4l2_dev, "dma2d isr %x\n",
	    	    dma2d_get_int(dev));
	}
	return IRQ_HANDLED;
}
