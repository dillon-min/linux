...
#define SRC_PIXEL_FORMAT	V4L2_PIX_FMT_RGB24
//#define OUT_PIXEL_FORMAT	V4L2_PIX_FMT_ARGB444
#define OUT_PIXEL_FORMAT	V4L2_PIX_FMT_RGB24
/*
...
int main(int argc, void *argv[])
{
	...
	printf("mmap framebuffer ok, fd %d, addr 0x%x\n", fb_fd, fbuffer);
	...
	fmt.fmt.pix.sizeimage	= SRC_WIDTH * SRC_HEIGHT * 3;
	...
	printf("mmap dma2d dest ok, fd %d, addr 0x%x\n", fd_video, p_dst_buf);
	buf.type	= V4L2_BUF_TYPE_VIDEO_CAPTURE;
	buf.memory	= V4L2_MEMORY_MMAP;
	buf.index	= 0;
	buf.bytesused	= SRC_WIDTH*SRC_HEIGHT*3;
	...
	ret = ioctl(fd_video, VIDIOC_G_FMT, &fmt);
	if (ret != 0) {
		printf("VIDIOC_G_FMT src failed 0x%x\n", errno);
		close(fd_video);
		return -1;
	}
	/* config src */
	...
	buf.memory	= V4L2_MEMORY_MMAP;
	buf.index	= 0;
	printf("num src bufs is %d, fd_video %d\n", num_src_bufs, fd_video);
	ret = ioctl(fd_video, VIDIOC_QUERYBUF, &buf);
	if (ret != 0) {
	...
	printf("mmap dma2d src ok, fd %d, addr 0x%x\n", fd_video, p_src_buf);
	...
	write(out_fd, p_src_buf, capture_buffer_sz);
	close(out_fd);
	memcpy(fbuffer, p_src_buf, capture_buffer_sz);
