...
static int dma2d_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_COLORFX:
		if (ctrl->val == V4L2_CID_COLORFX_ARGB) {
			frm = get_frame(ctx, V4L2_BUF_TYPE_VIDEO_CAPTURE);
			frm->a_rgb[3] = (ctx->colorfx_argb->val >> 24) & 0xff;
			frm->a_rgb[2] = (ctx->colorfx_argb->val >> 16) & 0xff;
			frm->a_rgb[1] = (ctx->colorfx_argb->val >>  8) & 0xff;
...
static int dma2d_setup_ctrls(struct dma2d_ctx *ctx)
{
	...
	v4l2_ctrl_new_custom(handler, &dma2d_r2m_control[0], NULL);
	ctx->colorfx = v4l2_ctrl_new_std_menu(handler, &dma2d_ctrl_ops,
					      V4L2_CID_COLORFX,
					      V4L2_COLORFX_SET_ARGB,
					      ~0x10000, V4L2_COLORFX_NONE);

	ctx->colorfx_argb = v4l2_ctrl_new_std(handler, &dma2d_ctrl_ops,
					      V4L2_CID_COLORFX_ARGB, 0,
					      0xffffffff, 1, 0);
